    ...
    return (
        <>
         <div className="box-1"> 
            ...
            { categoryBlogs.length === 0 ? <span className="text-light">Không có danh mục nào</span> : categoryBlogs.map( (item: any) => <span onClick={() => handleCategory(item)} className="title-blog-name color" key={item._id} >{item.name}</span>)}
